...
async function createCreditsProvider(spaceId: string): Promise<{ provider: AIProvider; modelName: string }> {
  ...
  return { provider: new LemonadeAIProvider(modelName, spaceId), modelName };
}
...
async function main(): Promise<void> {
  ...
  if (aiMode === 'own_key') {
    ...
    const result = await createCreditsProvider(defaultSpace);
